...
def cfg():
    return AppConfig(
        base_url="https://boc.test", username="u", password="p", request_timeout=5, max_retries=2
    )
...
async def test_get_injects_auth_headers(cfg):
    client, tm = _mkclient(
        cfg, [("GET", "https://boc.test/api/foo", _r(200, {"state": "success", "data": {"x": 1}}))]
    )
...
async def test_401_triggers_relogin_and_retry(cfg):
    client, tm = _mkclient(
        ...
        [
            (
                "GET",
                "https://boc.test/api/foo",
                _r(401, {"state": "error", "message": "token expired"}),
            ),
...
async def test_http_errors_map(cfg, status, exc):
    client, _ = _mkclient(
        cfg, [("GET", "https://boc.test/api/x", _r(status, {"message": f"e{status}"}))]
    )
...
async def test_timeout_raises_timeout_error(cfg):
    client, _ = _mkclient(cfg, [("GET", "https://boc.test/api/x", TimeoutError())])
