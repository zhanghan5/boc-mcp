...
from boc_mcp.services.cluster.models import ClusterNode, ClusterPartition, ClusterSummary
...
async def test_list_clusters():
    ...
    c.seed(
        "/cluster/v2.3/list",
        {
            "rows": [{"id": 1, "name": "c1"}, {"id": 2, "name": "c2"}],
            "totalCount": 2,
            "currPageNum": 1,
            "pageSize": 20,
            "pageCount": 1,
...
async def test_list_nodes():
    c = FakeClient()
    c.seed(
        "getClusterNodes", {"rows": [{"id": 1, "name": "n1", "ip": "10.0.0.1"}], "totalCount": 1}
    )
    svc = ClusterService(c)
