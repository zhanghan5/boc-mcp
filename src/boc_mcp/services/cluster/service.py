...
class ClusterService(BaseService):
    ...
    async def get_monitor_info(self, cluster_id: int) -> dict[str, Any]:
        resp = await self._get(f"/cluster/v3.0/listMonitorInfo/{cluster_id}")
        return resp.get("data") or resp

    async def get_state(self, cluster_id: int) -> dict[str, Any]:
        resp = await self._get(f"/platformCluster/v2.3/getClusterState/{cluster_id}")
        return resp.get("data") or resp

    async def list_nodes(
        self, cluster_id: int, node_type: str | None = None, page: int = 1, page_size: int = 20
    ) -> ListResult[ClusterNode]:
        body: dict[str, Any] = {
    ...
    async def list_platform_partitions(
        self, cluster_id: int, page: int = 1, page_size: int = 20
    ) -> ListResult[ClusterPartition]:
        body: dict[str, Any] = {
            "clusterId": cluster_id,
            "currPageNum": page,
            "pageSize": min(page_size, 100),
    ...
    async def get_partition_resource(self, host_ids: str) -> PartitionResource:
        ...
        data = resp.get("data") or {}
        return (
            PartitionResource.model_validate(data) if data else PartitionResource(host_ids=host_ids)
        )
...
def _parse_list[M: BaseModel](resp: dict[str, Any], model_cls: type[M]) -> ListResult[M]:
    if isinstance(resp.get("data"), list) and "rows" not in resp:
        items = [model_cls.model_validate(d) for d in resp["data"]]
        return ListResult[M](items=items, total=len(items), page_count=1)
