...
class BocApiClient(Protocol):
    ...
    def paginated_get(self, path: str, *, page_size: int = 20, **kwargs: Any) -> AsyncIterator[dict]: ...


_STATUS_MAP: dict[int, type[APIError]] = {
    400: BadRequestError, 401: AuthError, 403: ForbiddenError,
    ...
    async def request(self, method: str, path: str, *, params: dict | None = None,
        ...
        try:
            async with self._session.request(method, url, params=params, json=json,
                ...
                token_invalid = status == 401 or (isinstance(body, dict) and
                    is_token_invalid_message(str(msg)))
