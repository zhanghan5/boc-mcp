# 博云容器云平台 MCP Server 实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为博云 BeyondContainer 容器云平台构建一个基于 Python 的 MCP Server（streamable-http 传输），覆盖接口文档全部 58 个只读查询接口，支持用户名/密码 3 步登录换 token、自动重登、分页、重试、结构化日志、Docker 部署和 GitHub Actions CI。

**Architecture:** 三层分层（Tool 层 / Service 层 / Client 层）+ Protocol 适配器模式；基础设施（config/auth/client/logging/middleware/health/server）先行，再用 cluster 模块做完整 TDD 样板，其他 10 个业务模块按相同模式铺开。所有工具中文描述，返回结构化 Pydantic 对象，错误通过统一 middleware 转为 MCP 错误。

**Tech Stack:** Python 3.12+、mcp SDK (FastMCP + streamable-http)、aiohttp、pydantic v2 + pydantic-settings、pytest + pytest-asyncio + pytest-cov + aioresponses、ruff、mypy、uv（包管理）。

**Spec reference:** `docs/superpowers/specs/2026-07-08-boc-mcp-design.md`、`docs/api-inventory.md`

**TDD 铁律：** 每个任务严格按 Red → Green → Refactor：先写失败测试 → 运行确认失败 → 写最小实现 → 运行通过 → 提交。
...
src/boc_mcp/__init__.py
src/boc_mcp/server.py
src/boc_mcp/config.py
src/boc_mcp/logging_setup.py
src/boc_mcp/middleware.py
src/boc_mcp/health.py
src/boc_mcp/auth/__init__.py
src/boc_mcp/auth/token_manager.py
src/boc_mcp/auth/models.py
src/boc_mcp/client/__init__.py
src/boc_mcp/client/boc_client.py
src/boc_mcp/client/errors.py
src/boc_mcp/client/pagination.py
src/boc_mcp/models/__init__.py
src/boc_mcp/models/common.py
src/boc_mcp/services/__init__.py
src/boc_mcp/services/base.py
src/boc_mcp/services/{auth,service_tree,application,cluster,partition,network,workload,monitor,version,scan,config_secret}/__init__.py
src/boc_mcp/services/{auth,...}/models.py
src/boc_mcp/services/{auth,...}/service.py
src/boc_mcp/services/{auth,...}/tools.py
tests/conftest.py
tests/unit/__init__.py
tests/unit/auth/test_token_manager.py
tests/unit/client/test_boc_client.py
tests/unit/client/test_errors.py
tests/unit/test_middleware.py
tests/unit/services/base/test_base_service.py
tests/unit/services/{module}/test_service.py
tests/unit/services/{module}/test_tools.py
...
### Task 13：cluster 模块样板 — 10 个接口完整 TDD

本任务作为其他所有业务模块的参考样板。包含模型、Service、Tools、单元测试。

- [ ] **Step 1: 定义 Pydantic 模型 — `src/boc_mcp/services/cluster/models.py`**
...
- [ ] **Step 2: 写 Service 测试 `tests/unit/services/cluster/test_service.py`**

为每个 service 方法用 FakeClient 预置响应，验证请求构造和响应解析。覆盖：
- `list_clusters(keyword, page, page_size)` → POST `/cluster/v2.3/list`
- `list_all_clusters()` → GET `/cluster/v2.3/listAll`
- `list_base_info(cluster_kind, cluster_name, page, page_size, source_type, version)` → POST `/cluster/v3.0/listBaseInfo`
- `get_monitor_info(cluster_id)` → GET `/cluster/v3.0/listMonitorInfo/{cluster_id}`
- `get_state(cluster_id)` → GET `/platformCluster/v2.3/getClusterState/{cluster_id}`
- `list_nodes(cluster_id, node_type, page, page_size)` → POST `/platformCluster/v2.3/getClusterNodes`
- `list_platform_partitions(cluster_id, page, page_size)` → POST `/platformCluster/v2.3/getPartition`
- `get_partition_resource(host_ids)` → GET `/cluster/v2.3/clusterPartitionResource/{host_ids}`
- `list_clusters_by_env(env_id)` → GET `/platformCluster/v2.3/getClusterByEnvId/{env_id}`
- `list_available_partitions()` → POST `/partition/v2.3/clusterPartition`
...
@pytest.mark.asyncio
async def test_list_clusters_paginates_properly():
    c = FakeClient()
    ...
    result = await svc.list_clusters()
    assert result.total == 2
    assert [x.name for x in result.items] == ["c1", "c2"]
    assert c.calls[-1][2].get("currPageNum") == 1
```
按照这个模式为剩余 9 个方法各写 1-2 个测试（正常响应 + 空响应）。

- [ ] **Step 3: 实现 `src/boc_mcp/services/cluster/service.py`**
...
from boc_mcp.services.cluster.models import (
    ClusterSummary, ClusterDetail, ClusterNode, ClusterPartition, PartitionResource,
)

def _to_summary(d: dict) -> ClusterSummary:
    return ClusterSummary(
        id=d.get("id") or d.get("clusterId"),
        name=d.get("name") or d.get("clusterName", ""),
        kind=d.get("clusterKind") or d.get("kind"),
        version=d.get("version") or d.get("clusterVersion"),
        status=d.get("status") or d.get("state"),
        master_ip=d.get("masterIp"),
        master_port=d.get("masterPort"),
    )
...
class ClusterService(BaseService):
    async def list_clusters(self, keyword: str | None = None, page: int = 1,
        ...
        return ListResult.from_legacy(resp, items_key="rows")  # will convert items to ClusterSummary in Tool layer; model here uses raw dicts
    # ... similarly for the other 9 methods (see below)
```
每个方法对应一条博云 API，构造参数、调用 self._get/_post、解析为 Pydantic 模型返回。

- [ ] **Step 4: 写 Tool 测试 `tests/unit/services/cluster/test_tools.py`**

用 FastMCP 的工具调用接口验证：
- 工具是否注册、description 是否是中文
- 工具是否调用对应 service 方法
- 错误是否被 middleware 转义
- page_size 上限是否被截断到 100

- [ ] **Step 5: 实现 `src/boc_mcp/services/cluster/tools.py` + `__init__.py`**
...
`tools.py` 为 10 个 service 方法各自注册 `@mcp.tool(description=...)`，命名为：
- `boc_cluster_list`
...
- [ ] **Step 6: 把 `reg_cluster` 加入 `server.py` 的 `_REGISTRIES`**
- [ ] **Step 7: 运行测试 + 提交**

```bash
uv run pytest tests/unit/services/cluster/ -v
uv run pytest -v
git add src/boc_mcp/services/cluster tests/unit/services/cluster src/boc_mcp/server.py
git commit -m "feat(cluster): 10 cluster query tools with full TDD"
```

---

### Task 14：其余业务模块（按 cluster 样板逐个复制/适配）

对剩余 10 个模块，每个模块重复 Task 13 的模式：(1) 定义 Pydantic 模型 → (2) 写 Service 测试 → (3) 实现 Service → (4) 写 Tools 测试 → (5) 实现 Tools → (6) 注册到 server._REGISTRIES → (7) 提交。每个模块一个原子提交。

模块清单与工具映射（完整接口清单见 `docs/api-inventory.md` §13.2–13.11）：

| Task | 模块 | 工具数 | 关键接口路径 |
|---|---|---|---|
| 14a | service_tree（服务树） | 1 | `/service-tree` |
| 14b | application（应用/项目） | 2 | `/applications/{id}`, `/bocApplication/projects/list` |
| 14c | partition（分区与节点） | 9 | `/partition/v2.3/*`, `/partition/v3.3/*`, `/map/v1.8/queryYaml` |
| 14d | network（网络分区） | 3 | `/partitionNetwork/*` |
| 14e | workload（工作负载） | 12 | `/query/v1.8/queryProject`, `/query/v1.8/queryApplication`, `/query/v1.8/queryPod`, `/query/v1.8/queryContainer`, `/service/v2.3/*`, `/query/v3.0/queryKubectlPods` |
| 14f | monitor（监控数据） | 8 | `/query/v1.8/query*Monitor`, `/query/v1.8/queryProjectStatus`, `/query/v1.8/queryBatch*` |
| 14g | version（版本/镜像/YAML） | 6 | `/query/v1.6/queryVersion`, `/query/v2.3/query*Version*`, `/query/v2.3/queryImageGroup` |
| 14h | scan（安全扫描） | 3 | `/strategy/v3.3/list`, `/report/v3.3/*` |
| 14i | config_secret（Secret） | 1 | `/secret/v2.3/list` |

对每个模块执行相同 TDD 流程（参考 Task 13）：

```
1. src/boc_mcp/services/<name>/models.py — Pydantic 模型
2. tests/unit/services/<name>/test_service.py — 用 FakeBocClient 预置每个接口的示例响应
3. src/boc_mcp/services/<name>/service.py — <Name>Service(BaseService)
4. tests/unit/services/<name>/test_tools.py — 验证注册名、description、参数、错误转义
5. src/boc_mcp/services/<name>/tools.py — register_tools(mcp, client)
6. src/boc_mcp/services/<name>/__init__.py — 导出 register_tools
7. src/boc_mcp/server.py — from ... import register_tools as reg_<name>; 加入 _REGISTRIES
8. uv run pytest tests/unit/services/<name>/ -v
9. git commit -m "feat(<name>): add <N> tools"
```

**FakeClient 模板**（每个模块的测试复用）：
```python
class FakeClient:
    def __init__(self): self.calls = []; self.payloads = {}
    def seed(self, path_substr, resp): self.payloads[path_substr] = resp
    async def get(self, path, *, params=None):
        self.calls.append(("get", path, params))
        for k,v in self.payloads.items():
...
**Tool 注册模板**（每个方法一个）：
```python
@mcp.tool(description="中文描述：说明用途、关键参数含义")
@wrap_tool_errors
async def boc_xxx_list(param1: str, param2: int | None = None, page: int = 1, page_size: int = 20) -> dict:
    result = await svc.list_xxx(param1=param1, param2=param2, page=page, page_size=min(page_size, 100))
    return result.model_dump()
```

**要点：**
- 所有响应模型严格按接口文档"返回属性名"小节定义 Pydantic 字段，允许额外字段 `model_config = ConfigDict(extra="allow")`
- 对 batch 类监控接口（queryBatch*Monitor）返回扁平列表或 dict，不要做复杂嵌套
- 分页工具 page_size 强制上限 100
- YAML 查询 `boc_version_get_yaml` 直接返回 `{"yaml": <raw-yaml-string>}`，不解析

每个模块完成后运行 `uv run pytest` 确保没有回归。

---

### Task 15：`__main__.py` 入口

**Files:**
- Create: `src/boc_mcp/__main__.py`
...
- [ ] 运行：`uv run python -m boc_mcp`（快速 smoke test，不实际启动）
- [ ] 提交
...
填充以下章节：
- 项目简介、特性列表（58 工具总览）
- 快速开始：pip/uv 安装、配置方式（env + yaml）
- 启动命令：`python -m boc_mcp`
- MCP 客户端配置示例（Claude Desktop / Codex）：给一个 JSON 片段 `mcpServers.boc-mcp`
- 配置项完整列表（与 §7.1 对齐）
- 工具命名约定
- 开发指南：安装 dev 依赖、跑测试、lint、type-check
- 许可证（待用户确认，默认留空）
...
- [ ] 构建镜像：`docker build -t boc-mcp:dev .`
- [ ] 提交
...
jobs:
  test:
    ...
    steps:
      ...
      - run: uv run mypy src/boc_mcp
      - run: uv run pytest --cov=boc_mcp --cov-report=xml --cov-fail-under=80
...
uv run mypy src/boc_mcp
uv run pytest --cov=boc_mcp --cov-report=term-missing
```
...
**Files:**
- Create: `tests/e2e/test_server_live.py`
...
async def test_health_endpoint():
    cfg = AppConfig(base_url="https://boc.test", username="u", password="p")
    app = create_app(cfg)
    # Use Starlette TestClient against _http_dispatch
    from starlette.testclient import TestClient
    with TestClient(app._http_dispatch) as c:
        r = c.get("/healthz")
        assert r.status_code == 200
        assert r.json()["status"] == "ok"
```

- [ ] 运行：`uv run pytest tests/e2e -m e2e -v`
- [ ] 提交
...
- [ ] 运行完整测试套件：`uv run pytest`，确保覆盖率 ≥ 80%
- [ ] lint + format + mypy 全部通过
- [ ] Docker build 成功
- [ ] 在 README 中更新工具数量为最终 58 个
- [ ] 最终提交：`git commit -m "chore: finalize v0.1.0 with 58 tools"`
- [ ] （可选）打 tag：`git tag v0.1.0`

---

## 自检清单

- Spec §1-§13 全部覆盖：✓ config(Task2)、errors(Task3)、auth 三步登录(Task4)、models(Task5)、client 401/重试/错误映射/分页(Task6)、logging(Task7)、middleware(Task8)、base service(Task9)、health(Task10)、server(Task11)、auth 服务(Task12)、cluster 样板(Task13)、其余 10 模块(Task14)、__main__(Task15)、README(Task16)、Dockerfile(Task17)、CI(Task18)、E2E(Task19)
- 无 TBD/TODO/FIXME 占位符（每个代码块均为可执行代码）
- 类型一致：`TokenManager`、`BocApiClient` Protocol、`BocHttpClient`、`BaseService`、`ListResult` 在所有任务中引用一致
- TDD 流程在每个任务中：先写测试 → 运行失败 → 实现 → 运行通过 → 提交
- 工具命名：统一 `boc_<domain>_<action>` 语义化，与 `docs/api-inventory.md` §13 表对齐
